...
**Linear (optional):** Script: `python scripts/create_linear_ticket_invite_login.py` (vereist `LINEAR_API_KEY` in .env).
...
- Lookup of balance list by invite code required a token, so the list could not be shown before login
...
4. **Activation flow**: First login allowed without activation; email verification required for subsequent logins. Activating = set/use password so you can log in again.
...
Create a `.env` file in the project root (never commit secrets). Required for auth:

```
SECRET_KEY=<min-32-chars-strong-secret>
...
# Used for activation email links and redirect-after-error
...
- `GOOGLE_REDIRECT_URI` must match exactly one of the "Authorized redirect URIs" in Google Cloud Console (e.g. frontend URL so the browser lands on the app with `?code=...`).
- `FRONTEND_URL` is used when the backend redirects the user after a Google callback error (e.g. 403 needs_invite).

### Production: no .env file

In production you **do not** use a `.env` file. Set the same variables as **environment variables** in your hosting platform:

| Platform | Where to set env vars |
|----------|------------------------|
| **Render** | Dashboard → Your Service → Environment → Add variable |
| **Railway / Fly.io / Heroku** | Project Settings → Variables / Config Vars |
| **Docker** | `docker run -e VAR=value` or `env_file:` in compose |
| **Cloudflare Workers** | Dashboard → Workers → Settings → Variables |
| **Vercel** | Project → Settings → Environment Variables |

Production values to set:

- `SECRET_KEY` – strong random secret (≥32 chars)
- `GOOGLE_CLIENT_ID` – same or separate OAuth client for prod
- `GOOGLE_CLIENT_SECRET` – prod client secret
- `GOOGLE_REDIRECT_URI` – **production frontend URL** where Google redirects after login, e.g. `https://better-wbw.arjantb92.workers.dev/`
- `FRONTEND_URL` – production frontend origin, e.g. `https://better-wbw.arjantb92.workers.dev`
- `DATABASE_URL` / `DIRECT_URL` – production database URLs

Add the **production** redirect URI and JavaScript origin to your Google OAuth client (Authorized redirect URIs / Authorized JavaScript origins) so Google accepts the prod domain.
...
3. **OAuth state parameter** (JWT with invite_code) prevents CSRF
4. **Google emails** are auto-verified (Google handles verification)
5. **Password requirements**: Minimum 6 characters
6. **Lookup**: `GET /balance-lists/lookup/<invite_code>` stays token-required for logged-in users; public preview is a separate open endpoint.
